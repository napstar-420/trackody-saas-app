...
export function NetworkSummary() {
  ...
  const [colors] = useState(
    theme === 'light' ? ['#f6f1e9', '#4fc9da'] : ['#2b2b40', '#4fc9da']
  );
  ...
  return (
    <div className='bg-[#fdfcfb] dark:bg-dark_500 border border-dashed dark:border-dark_400 py-4 rounded-xl px-8'>
      ...
      <div className='py-4 grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] xl:grid-cols-4'>
        {summary.map((network, index) => {
          ...
          return (
            <div key={index}>
              <h1 className='text-light_800 dark:text-gray-400 font-medium'>
                {label}
              </h1>
              <h2 className='text-3xl text-light_text dark:text-white font-semibold flex items-center gap-2'>
                {value}{' '}
                <span
                  className={`text-sm ${
              ...
              <h3 className='text-sm text-light_600 dark:text-gray-500 font-medium'>
                Last {period} Days
              </h3>
              <Chart
                options={options}
